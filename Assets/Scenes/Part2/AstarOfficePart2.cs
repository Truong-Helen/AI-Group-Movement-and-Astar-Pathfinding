...
public class AstarOfficePart2 : MonoBehaviour
{
    public static AstarOfficePart2 inst;
    private void Awake()
    {
        inst = this; 
    }
    ...
    public Transform officeCenterParent;
    ...
    void Start()
    {
        ...
        int count = 0;
        foreach (Transform office in officeCenterParent.transform)
        {

            if (count < officeCenterParent.childCount-1)
            {
                count++;
                startPoints.Add(office.position);
            }
            if (count > 1)
            {
                endPoints.Add(office.position);
            }
        }
        ...
        if (SelectionMgr.inst.selectedEntity != null)
        {
            ...
            if (Input.GetMouseButtonDown(1))
            {
                ...
                if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, float.MaxValue, endLayerMask))
                {
                    ...
                    endPoints.Add(endPointPosition);

                    startPoints.Insert(0, startPointPosition);

                    for (int i = 0; i < startPoints.Count; i++)
                    {
                        startPointPosition = startPoints[i];
                        endPointPosition = endPoints[i];
                        GeneratePathWaypoints(FindPath());
                        
                        if (smoothPath.Count > 0)
                        {
                            for (int j = 0; j < smoothPath.Count; j++)
                            {
                                Debug.Log("Move to: " + smoothPath[j].worldPosition);
                                MoveToCheckpoint(smoothPath[j].worldPosition);
                            }
                            smoothPath.Clear();
